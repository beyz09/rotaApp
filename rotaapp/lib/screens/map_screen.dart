...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _performSearch(String query) async {
      ...
      if (currentLocation != null) {
        ...
        if (mounted) {
          setState(() {
           ...
           try { _mapController.move(currentLocation, 13); } catch (_) {}
        }
      } else {
    ...
    if (routeProvider.routeOptionsList.isNotEmpty) {
       routeProvider.clearRouteResults();
        if (mounted && _currentSheet == SheetType.routeOptions) {
           setState(() { _currentSheet = SheetType.none; _showRouteStepsDetails = false; });
        }
    }
    ...
    try {
      final url = Uri.parse('https://nominatim.openstreetmap.org/search?format=json&q=${Uri.encodeComponent(query)}&limit=10&viewbox=$turkeyViewBox&bounded=1&accept-language=tr');
      final response = await http.get(url, headers: {'User-Agent': 'FuelEstimateApp/1.0'}).timeout(const Duration(seconds:10));
      ...
      if (response.statusCode == 200) {
        ...
        results = data.map((item) {
          ...
          if (lat != null && lon != null && name.isNotEmpty) {
            return LocationResult(displayName: name, coordinates: LatLng(lat, lon), type: item['type']?.toString() ?? '');
          }
          return null;
      ...
      _showErrorSnackBar('Arama sırasında bir hata oluştu.'); // ${e.toString()} kaldırıldı
    }
  }
  ...
  Future<List<RouteOption>> _fetchRouteOptions(LatLng start, LatLng end) async {
      final url = Uri.parse('http://router.project-osrm.org/route/v1/driving/${start.longitude},${start.latitude};${end.longitude},${end.latitude}?overview=full&geometries=geojson&alternatives=true&steps=true&annotations=true');
      
      http.Response response;
      try {
        response = await http.get(url).timeout(const Duration(seconds: 30)); // Zaman aşımı biraz daha artırıldı
      } on TimeoutException catch (_) {
      ...
      if (response.statusCode != 200) {
          ...
          if (response.statusCode == 400) { 
              if (response.body.toLowerCase().contains("too big") || response.body.toLowerCase().contains("retaillimit")) { // OSRM bazen retaillimit hatası verebilir
      ...
      final calculator = selectedVehicle != null
          ? FuelCostCalculator(vehicle: selectedVehicle, fuelPricePerLiter: _sampleFuelPricePerLiter, cityPercentage: cityPercentageForFuel)
          : null;
      ...
      for (var routeIndex = 0; routeIndex < data['routes'].length; routeIndex++) {
          ...
          final String routeName = routeIndex == 0 ? 'En Hızlı Rota' : 'Alternatif Rota ${routeIndex + 1}';
          ...
          if (routeData['legs'] != null && routeData['legs'].isNotEmpty && routeData['legs'][0]['steps'] != null) {
          ...
          final List<String> tollSegmentsDescriptions = tollResult['segments'] as List<String>;
  ...
  TollGate? _findClosestGate(LatLng coordinate, List<TollGate> gates, double thresholdMeters) {
    ...
    Map<String, dynamic> _calculateDetailedToll(List<RouteStep> steps) {
                ...
                if (closestEntryGate != null && closestExitGate != null) {
                    ...
                    if (closestEntryGate.name == closestExitGate.name) {
                        ...
                        if (tollCostsMatrix.containsKey(entryName) && tollCostsMatrix[entryName]!.containsKey(exitName)) {
                           ...
                           currentSegmentCostUnknown = true; // Bu spesifik segmentin maliyeti bilinmiyor
                        ...
                        hasTollRoadSectionVisible = true; // Bir segment bulundu (aynı olsalar bile)
                    }
                } else { // Gişelerden biri veya ikisi bulunamadı
        ...
        return {
           ...
           'segments': identifiedTollSegments.toSet().toList(), // ***** TEKRARLARI KALDIR *****
  ...
  Widget _buildLocationResultListItem(BuildContext context, LocationResult result) {
    ...
    Widget _buildRouteOptionListItem({
        ...
        shape: RoundedRectangleBorder(
            ...
            side: BorderSide(color: isSelected ? Theme.of(context).colorScheme.primary : Colors.grey[300]!, width: isSelected ? 1.5 : 1),
        ),
            ...
            child: Column(
                ...
                Row(
                    ...
                    Expanded(
                        child: Text(
                        route.name,
                        style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isSelected ? Theme.of(context).colorScheme.primary : Colors.black87),
                        maxLines: 1,
                    ...
                    if (hasToll)
                        ...
                        child: Tooltip(
                            message: tooltip,
                            child: Icon(Icons.toll, size: 20, color: costUnknown ? Colors.orange : (tollCost > 0 ? Colors.redAccent : Colors.grey)),
                        ),
                        ),
                    if (isSelected)
                        const Padding(
                    ...
                    children: [
                        ...
                        _buildRouteInfoItem(
                            ...
                            route.costRange != null
                                ...
                                : '-',
                            isSelected, context, highlight: true
                        ),
                    ],
                ),
                ],
    ...
    Widget _buildRouteInfoItem(IconData icon, String label, String value, bool isSelected, BuildContext context, {bool highlight = false}) {
        return Expanded(
        child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
    ...
    IconData _getManeuverIcon(String type, String? modifier) {
        switch (type) {
            ...
            case 'new name': return Icons.merge_type;
            case 'depart': return Icons.outbound;
            case 'arrive': return Icons.flag;
            case 'fork': return Icons.call_split;
            case 'merge': return Icons.merge_type;
            case 'ramp': return Icons.ramp_right;
            case 'roundabout': return Icons.roundabout_right;
            case 'end of road': return Icons.block;
    ...
    Widget _buildRouteDetailedCard(BuildContext context, RouteOption route) {
        ...
        return Card(
            ...
            child: Column(
                ...
                _buildDetailRow(Icons.speed, 'Araç', vehicle != null ? '${vehicle.brand} ${vehicle.model}' : 'Seçilmedi'),
                const SizedBox(height: 8),
                ...
                if (vehicle != null) ...[
                    ...
                    Card( color: Colors.orange[50], elevation: 0, child: Padding( padding: const EdgeInsets.all(8.0), child: Row( children: [ Icon(Icons.warning_amber_rounded, color: Colors.orange[700], size: 18,), const SizedBox(width: 8), Expanded(child: Text('Yakıt/toplam maliyet için araç seçimi gerekli.', style: TextStyle(color: Colors.orange[800], fontSize: 12),),), ], ), ), ),
                    const SizedBox(height: 8),
                     _buildDetailRow(Icons.toll, 'Gişe Durumu', tollStatusText, color: tollColor),
                      if (hasTollSection && segments.isNotEmpty)
                         Padding(
                ...
                // Geçilen şehirler kaldırıldı
                // if (route.intermediatePlaces.isNotEmpty) ...[
                //    const Divider(height: 24),
                //    Text('Tahmini Geçiş Noktaları', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.grey[800])),
                //    const SizedBox(height: 6),
                //    Text(route.intermediatePlaces.join(' → '), style: TextStyle(fontSize: 13, color: Colors.grey[600])),
                // ],
                        ...
                        return Padding(
                            ...
                            children: [
                                ...
                                child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                    Text(instruction, style: const TextStyle(fontSize: 13.5)),
                                    Text('${(step.distance / 1000).toStringAsFixed(1)} km', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                                    ],
                                ),
                                ),
                            ],
    ...
    Widget _buildDetailRow(IconData icon, String label, String value, {Color? color, bool isBold = false}) {
        ...
        child: Row(
            ...
            Expanded(
                child: Text(
                value,
                style: TextStyle(fontSize: 13.5, fontWeight: isBold ? FontWeight.bold : FontWeight.normal, color: color ?? Colors.black87),
    ...
    Widget build(BuildContext context) {
        ...
        double initialSheetSize = midSheetSize;
         if (_currentSheet == SheetType.routeOptions) initialSheetSize = maxSheetSize;
            ...
            FlutterMap(
                ...
                children: [
                    TileLayer(
                        ...
                        userAgentPackageName: 'com.example.rotaapp', // Kendi paket adınızla değiştirin
                    ),
                    if (routeProvider.selectedRouteOption != null)
                        PolylineLayer(
                        polylines: [
                    ...
                    children: [
                        ...
                        children: [
                             ...
                             Expanded(
                                ...
                                decoration: InputDecoration(
                                    hintText: 'Başlangıç', isDense: true, border: InputBorder.none,
                                    suffixIconConstraints: const BoxConstraints(maxHeight: 24),
                                    suffixIcon: _startController.text.isNotEmpty
                                        ? IconButton(icon: const Icon(Icons.clear, size: 18), onPressed: () { _startController.clear(); routeProvider.setStartLocation(null); if(mounted) setState(() => _searchResults=[]);}, padding: EdgeInsets.zero, constraints: const BoxConstraints(),)
                                        : (currentLocation != null && routeProvider.startLocation != currentLocation)
                                            ? IconButton(icon: const Icon(Icons.my_location, size: 18), tooltip: 'Mevcut Konum', onPressed: () { if(mounted) setState(() => _isStartSearchActive = true); _performSearch('Mevcut Konum'); }, padding: EdgeInsets.zero, constraints: const BoxConstraints(),)
                                            : null,
                                ),
                        ...
                        children: [
                            ...
                            Expanded(
                                ...
                                decoration: InputDecoration(
                                    hintText: 'Varış', isDense: true, border: InputBorder.none,
                                    suffixIconConstraints: const BoxConstraints(maxHeight: 24),
                                     suffixIcon: _endController.text.isNotEmpty
                                        ? IconButton(icon: const Icon(Icons.clear, size: 18), onPressed: () { _endController.clear(); routeProvider.setEndLocation(null); if(mounted) setState(() => _searchResults=[]); }, padding: EdgeInsets.zero, constraints: const BoxConstraints(),)
                                        : null,
                                ),
                                textInputAction: TextInputAction.search,
                                onSubmitted: (query) { if(query.isNotEmpty) _performSearch(query); },
                        ...
                        width: double.infinity,
                        child: ElevatedButton.icon(
                            icon: _isCalculatingRoute ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.directions, size: 20),
                            ...
                            style: ElevatedButton.styleFrom(
                                ...
                                padding: const EdgeInsets.symmetric(vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), elevation: 4,
                            ),
                            onPressed: (_isCalculatingRoute || routeProvider.startLocation == null || routeProvider.endLocation == null) ? null : _onRouteSearchRequested,
                        ),
                        ),
                    ],
                    ),
